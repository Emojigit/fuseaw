...
bool parse_akpk_file(std::istream &file, AKPKFileData &data) {
    ...
    if (!get_languages(file, data.header.languages_sector_size, data.language_data)) {
        return false;
    }
    ...
    if (!get_sector(file, data.header.banks_sector_size, false, false, "bnk", endianness, bank_version, data.sector_banks)) {
        return false;
    }

    if (bank_version == 0) bank_version = 62;

    if (!get_sector(file, data.header.sounds_sector_size, true, false, "wem", endianness, bank_version, data.sector_sounds)) {
        return false;
    }

    if (!get_sector(file, data.header.externals_sector_size, true, true, "wem", endianness, bank_version, data.sector_externals)) {
        return false;
    }

    for (const auto& entry : data.sector_banks) {
        if (strcmp(entry.file_extension, "bnk") != 0) continue;
