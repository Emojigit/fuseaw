...
// Compilation command:
// g++ -std=c++23 test_bnk_extract.cpp src/*.cpp -I./src -o test_bnk_extract
...
int main(int argc, char *argv[])
{
    ...
    std::string bnk_entry_id_str  = argv[3];
    ...
    // Parse the target file ID (handles both decimal and hex strings like 0x12A)
    uint64_t bnk_entry_id = 0;
    try
    {
        bnk_entry_id = std::stoull(bnk_entry_id_str, nullptr, 0);
    }
    catch (const std::exception& e)
    {
        std::cerr << "Error: Invalid bnk entry ID format '" << bnk_entry_id_str << "'.\n";
        return 1;
    }

    BNKFileMeta target_bnk_entry;
    bool bnk_found = false;

    for (const auto& file : bnk_files) {
        if (file.meta_raw.wem_id == bnk_entry_id) {
            target_bnk_entry = file;
            bnk_found = true;
            break;
        }
    }
    ...
    if (!bnk_found) {
        std::cerr << "Error: BNK entry ID " << bnk_entry_id << " could not be found.\n";
        return 1;
    }

    // Open output stream early to catch write permissions/path errors before extraction
    std::ofstream out_file(out_filename, std::ios::binary);
    if (!out_file.is_open())
    {
        std::cerr << "Error: Could not open output file '" << out_filename << "' for writing.\n";
        return 1;
    }

    if (target_bnk_entry.meta_raw.wem_size > 0)
    {
        std::vector<char> buffer(target_bnk_entry.meta_raw.wem_size);
        if (!get_bnk_file(file, target_bnk_entry, buffer.data()))
        {
            std::cerr << "Error: Failed to extract data from bnk file.\n";
            return 1;
        }
        ...
        if (!out_file)
        {
            std::cerr << "Error: Encountered an error while writing data to output file.\n";
            return 1;
        }
    }

    std::cout << "Successfully extracted file to '" << out_filename << "'\n";
