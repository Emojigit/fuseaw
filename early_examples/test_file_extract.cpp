...
// Compilation command:
// g++ -std=c++23 test_file_extract.cpp src/wavescan.cpp -I./src -o test_file_extract
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4)
    {
        std::cerr << "Usage: " << argv[0] << " <path_to_akpk_file> <file_id> <output_filename>\n";
        return 1;
    }
    ...
    std::cout << "Found target entry (Size: " << target_entry.file_size << " bytes). Extracting...\n";
    ...
    {
        std::vector<char> buffer(target_entry.file_size);
        if (!get_file(file, target_entry, buffer.data()))
        {
            std::cerr << "Error: Failed to extract data from pack file.\n";
            return 1;
        }
